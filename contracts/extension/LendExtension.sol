...
import {ILendExtension} from "../interfaces/internal/ILendExtension.sol";
import {INonfungiblePositionManager} from "../interfaces/external/INonfungiblePositionManager.sol";
contract LendExtension is ModuleBase,ILendExtension,Initializable,UUPSUpgradeable,ReentrancyGuardUpgradeable{ 
    ...
    function replacementLiquidity(address _holder,ReplacementLiquidityType _type,uint24 _fee,int24 _tickLower,int24 _tickUpper) external  nonReentrant onlyVault(_holder){
         ...
         if(_type==ReplacementLiquidityType.Put){
            ILendFacet.PutOrder memory putOrder = ILendFacet(diamond).getBorrowerPutOrder(_holder);
            require(putOrder.borrower != address(0),"LendModule:putOrder not exist");
            require(putOrder.collateralAssetType==1,"LendModule:collateralAssetType error"); 
            (tokenId,newLiquidity)= mintNewNft(putOrder.borrower,putOrder.collateralAsset,putOrder.collateralNftId,_fee,priceSection);
            ILendFacet(diamond).setBorrowerPutOrderNftInfo(putOrder.borrower,tokenId,uint256(newLiquidity));
            emit ReplacementLiquidity(_type,putOrder.borrower,_fee,_tickLower,_tickUpper, tokenId,newLiquidity);
         }else if(_type==ReplacementLiquidityType.Call){
            ILendFacet.CallOrder memory callOrder = ILendFacet(diamond).getLenderCallOrder(_holder);
            require(callOrder.lender != address(0),"LendModule:callOrder not exist");
            require(callOrder.collateralAssetType==1,"LendModule:collateralAssetType error");
            (tokenId,newLiquidity)= mintNewNft(callOrder.lender,callOrder.collateralAsset,callOrder.collateralNftId,_fee,priceSection);
            ILendFacet(diamond).setLenderCallOrderNftInfo(callOrder.lender,tokenId,uint256(newLiquidity));
            emit ReplacementLiquidity(_type,callOrder.lender,_fee,_tickLower,_tickUpper, tokenId,newLiquidity);
         }else{
