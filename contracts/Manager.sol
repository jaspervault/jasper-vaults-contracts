...
contract Manager is  Initializable, UUPSUpgradeable, ReentrancyGuardUpgradeable{
   ...
   function deleteFacetAllSelector(address _facetAddress) external onlyOwner{
    ...
    //-----------
    function getVaultAllPosition(address _vault,uint16[] memory _positionTypes) external view returns(IVaultFacet.Position[] memory positions,uint256[] memory decimals){
             ...
             for(uint256 i;i<positions.length;i++){
                 if(positions[i].component == IPlatformFacet(diamond).getEth()){
                   ...
                   decimals[i]=18;
                     ...
                     positions[i].balance=IERC20(positions[i].component).balanceOf(_vault);
    ...
    function getVaultProtocolPosition(address _vault,uint16 _positionType) external view returns(IVaultFacet.Position[] memory positions,uint256[] memory decimals){
    ...
    function getLenderPutOrder(address _lender) external view returns(address[] memory){
         return ILendFacet(diamond).getLenderPutOrder(_lender);
    }

    function getBorrowerPutOrder(address _borrower) external view returns(ILendFacet.PutOrder memory){
         return  ILendFacet(diamond).getBorrowerPutOrder(_borrower);
    }
    //
    ...
    //    
    function getLenderCallOrder(address _lender) external view returns(ILendFacet.CallOrder memory){
        return ILendFacet(diamond).getLenderCallOrder(_lender);
    }
    function getBorrowerCallOrder(address _borrower) external view returns(address[] memory) {
        return ILendFacet(diamond).getBorrowerCallOrder(_borrower);
    }
