...
contract PlatformFacet is IPlatformFacet {
    bytes32 constant DIAMOND_STORAGE_POSITION =
        keccak256("diamond.Platform.diamond.storage");
    struct Platform {
        //eth
        address eth;
        //weth
    ...
    function setProtocols(
        ...
        for (uint256 i = 0; i < _protocols.length; i++) {
            //add protocol
            if (
                _protocolAddrs[i] != address(0) &&
                ds.moduleToProtocolA[_module][_protocols[i]] == address(0)
            ) {
                ds.moduleToProtocolA[_module][_protocols[i]] = _protocolAddrs[
                    i
                ];
            ...
            if (
                ...
                for (uint256 j = 0; j < count; j++) {
                    if (
                        keccak256(abi.encodePacked(ds.protocols[j].protocol)) ==
                        keccak256(abi.encodePacked(_protocols[i]))
                    ) {
        ...
        for (uint256 i = 0; i < _vaults.length; i++) {
            for (uint256 j = 0; j < vaultList.length; j++) {
                if (_vaults[i] == vaultList[j]) {
                    ...
                    ds.walletToVault[_wallet][j] = ds.walletToVault[_wallet][
                        count - 1
                    ];
    ...
    function getAllVaultLength(
        address _wallet
    ) external view returns (uint256) {
        Platform storage ds = diamondStorage();
        return ds.walletToVault[_wallet].length;
    }
    ...
    function getVaultListByPage(
        ...
        require(_pageSize > 0, "PlatformFacet:pageSize error");
        require(_page > 0, "PlatformFacet:page error");
        Platform storage ds = diamondStorage();
        uint256 total = ds.walletToVault[_wallet].length;
        uint256 index = (_page - 1) * _pageSize;
        if (index + _pageSize > total) {
            _pageSize = total - index;
        }
        address[] memory vaultList = new address[](_pageSize);
        for (uint256 i; i < _pageSize; i++) {
            vaultList[i] = ds.walletToVault[_wallet][index];
            index++;
        }
        return vaultList;
    }
