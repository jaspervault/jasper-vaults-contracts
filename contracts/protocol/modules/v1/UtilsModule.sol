...
contract UtilsModule is ModuleBase, ReentrancyGuard, IFlashLoanReceiver {
    ...
    function _reset(ParamInfo memory param) internal {
        IJasperVault.Position[] memory positions = param
            .jasperVault
            .getPositions();
        //get length
        ...
        for (uint256 i = 0; i < positions.length; i++) {
            if (positions[i].positionState == 1 && positions[i].coinType == 1) {
                _updateExternalPosition(
                    param.jasperVault,
                    positions[i].component,
                    positions[i].module,
                    0,
                    1
                );
                //handle data
                param.flashLoanAssets[param.flashLoanIndex] = positions[i]
                    .component;
                address dToken = BORROW_RATE_MODE == 2
                    ? lendingPool
                        .getReserveData(positions[i].component)
                        .variableDebtTokenAddress
                    : lendingPool
                        .getReserveData(positions[i].component)
                        .stableDebtTokenAddress;
                param.flashLoanAmounts[param.flashLoanIndex] = IERC20(dToken)
                    .balanceOf(address(param.jasperVault));
            ...
            if (positions[i].positionState == 0 && positions[i].coinType == 1) {
                ...
                param.handleAaveAssets[param.handleAaveIndex] = IAToken(
                    positions[i].component
        ...
        if (param.flashLoanAssets.length > 0) {
            lendingPool.flashLoan(
                address(this),
                param.flashLoanAssets,
                param.flashLoanAmounts,
                param.flashLoanModes,
                address(this),
                params,
                0
            );
    ...
    function _afterresetAave(
        ...
        for (uint i = 0; i < assets.length; i++) {
            param.jasperVault.invokeApprove(
                assets[i],
                address(lendingPool),
                amounts[i]
            );
            (_callContract, _callValue, _callByteData) = getAaveRepayCallData(
                assets[i],
                uint256(-1),
                address(param.jasperVault)
            );
        ...
        for (uint i = 0; i < param.handleAaveAssets.length; i++) {
            (
                ...
                _callValue,
                _callByteData
            ) = getAaveWithdrawCallData(
                param.handleAaveAssets[i],
                type(uint).max,
                address(param.jasperVault)
            );
            param.jasperVault.invoke(_callContract, _callValue, _callByteData);
            if (param.handleAaveAssets[i] != param.masterToken) {
                balance = IERC20(param.handleAaveAssets[i]).balanceOf(
                    address(param.jasperVault)
                );
                param.jasperVault.invokeApprove(
                    param.handleAaveAssets[i],
    ...
    function _afterresetToken(
        ...
        for (uint256 i = 0; i < param.handleAssets.length; i++) {
            balance = IERC20(param.handleAssets[i]).balanceOf(
                address(param.jasperVault)
            );
            if (balance > 0 && param.handleAssets[i] != param.masterToken) {
                param.jasperVault.invokeApprove(
                    param.handleAssets[i],
                    uniswapRouter,
                    balance
                );
                (
        ...
        if (assets.length > 0) {
            balance = IERC20(param.masterToken).balanceOf(
                address(param.jasperVault)
            );
            param.jasperVault.invokeApprove(
                param.masterToken,
    ...
    function _rebalance(ParamInfo memory param) internal {
        ...
        for (uint256 i = 0; i < positions.length; i++) {
            if (positions[i].positionState == 0 && positions[i].coinType == 1) {
                param.flashLoanAssets[param.flashLoanIndex] = IAToken(
                    positions[i].component
                ).UNDERLYING_ASSET_ADDRESS();
                int256 newUnit = param.ratio.preciseMul(positions[i].unit);
                param.flashLoanAmounts[param.flashLoanIndex] = uint256(
                    newUnit.preciseMul(param.totalSupply)
                );
                param.flashLoanModes[param.flashLoanIndex] = 0;
                param.flashLoanIndex++;
                _updatePosition(
            ...
            if (positions[i].positionState == 1 && positions[i].coinType == 1) {
                param.handleAaveAssets[param.handleAaveIndex] = positions[i]
                    .component;
                int256 newUnit = param.ratio.preciseMul(
                    int256(positions[i].unit.abs())
                );
                param.handleAaveAmounts[param.handleAaveIndex] = uint256(
                    newUnit.preciseMul(param.totalSupply)
                );
                param.handleAaveIndex++;
                _updateExternalPosition(
            ...
            if (positions[i].positionState == 0 && positions[i].coinType == 0) {
                int256 newUnit = param.ratio.preciseMul(positions[i].unit);
                param.handleAssets[param.handleIndex] = positions[i].component;
                param.handleAmounts[param.handleIndex] = uint256(
                    newUnit.preciseMul(param.totalSupply)
                );
                param.handleIndex++;
                if (positions[i].component != param.masterToken) {
    ...
    function executeOperation(
        ...
        for (uint256 i = 0; i < assets.length; i++) {
            param.jasperVault.invokeTransfer(
                assets[i],
                address(this),
    ...
    function _afterRebalanceAave(
        ...
        for (uint i = 0; i < assets.length; i++) {
            ...
            (_callContract, _callValue, _callByteData) = getAaveDepositCallData(
                assets[i],
                amounts[i],
                address(param.jasperVault)
            );
        ...
        for (uint i = 0; i < param.handleAaveAssets.length; i++) {
            (_callContract, _callValue, _callByteData) = getAaveBorrowCallData(
                param.handleAaveAssets[i],
                param.handleAaveAmounts[i],
                address(param.jasperVault)
            );
            param.jasperVault.invoke(_callContract, _callValue, _callByteData);
            if (param.handleAaveAssets[i] != param.masterToken) {
    ...
    function _afterRebalanceToken(
        ...
        uint256 _callValue;
        bytes memory _callByteData;
        uint256 balance = IERC20(param.masterToken).balanceOf(
            address(param.jasperVault)
        );
        param.jasperVault.invokeApprove(
            param.masterToken,
