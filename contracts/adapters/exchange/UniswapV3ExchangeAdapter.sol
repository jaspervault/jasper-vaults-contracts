...
contract UniswapV3ExchangeAdapter is
    IExchangeAdapter,
    Initializable,
    UUPSUpgradeable,
    ReentrancyGuardUpgradeable
{
    ...
    function getAdapterCallData(
        ...
        bytes memory _data
    ) external view returns (AdapterCalldata memory _adapterCalldata) {
        (bytes memory path, uint160 sqrtPriceLimitX96, bool isEth) = abi.decode(
            _data,
            (bytes, uint160, bool)
        );
        ...
        _adapterCalldata.target = router;
        {
            if (_adapterType == 1) {
                ...
                _adapterCalldata.data = exactInputSingle(
                    _sendAsset,
                    _receiveAsset,
                    path.toUint24(20),
                    _vault,
                ...
                _adapterCalldata.data = exactOutput(
                    path,
                    _vault,
                    _amountIn,
                    _amountLimit
                ...
                _adapterCalldata.value = isEth ? _amountLimit : 0;
                _adapterCalldata.data = exactOutputSingle(
                    _sendAsset,
                    _receiveAsset,
                    path.toUint24(20),
                    _vault,
                ...
                _adapterCalldata.value = isEth ? _amountIn : 0;
                _adapterCalldata.data = exactInput(
                    path,
                    _vault,
                    _amountIn,
                    _amountLimit
    ...
    function validData(
        ...
        address tokenIn = _adapterType == 2
            ? path.toAddress(len)
            : path.toAddress(0);
        address tokenOut = _adapterType == 2
            ? path.toAddress(0)
            : path.toAddress(len);
        require(
            tokenIn == _sendAsset,
            "UniswapV3ExchangeAdapter:sendAsset must be same as path[0]"
        );
        require(
            tokenOut == _receiveAsset,
            "UniswapV2ExchangeAdapter:receiveAsset must be same as path[laset]"
        );

        for (uint i = 23; i < len; i++) {
            tokenIn = path.toAddress(i);
            require(
                platformFacet.getTokenType(tokenIn) != 0,
                "UniswapV3ExchangeAdapter:token must be platform allowed"
            );
            i += 23;
        }
    }
