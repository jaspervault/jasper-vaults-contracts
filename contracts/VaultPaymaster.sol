...
import "./eip/4337/interfaces/IPaymaster.sol";
...
import {IExchangeAdapter} from "./interfaces/internal/IExchangeAdapter.sol";
    ...
    ReentrancyGuardUpgradeable
{
    ...
    function setQuotaWhiteList(
        ...
        require(
            msg.sender == IOwnable(diamond).owner() ||
                msg.sender == IPaymasterFacet(diamond).getPayer(),
            "only owner"
        );
        IPaymasterFacet(diamond).setQuotaWhiteList(_type, _target, _amount);
    }

    function deposit(
        IPaymasterFacet.DepositInfo memory _depositInfo
    ) external nonReentrant {
        IPlatformFacet platformFacet = IPlatformFacet(diamond);
        address IPlatformAdapter = platformFacet.getModuleToProtocolA(
            address(this),
            _depositInfo.protocol
        );
        address weth = platformFacet.getWeth();
        require(
            IPlatformAdapter != address(0),
            "VaultPaymaster:protocol must be platform allowed"
        );
        uint256 sendAssetType = platformFacet.getTokenType(
            _depositInfo.sendAsset
        );
        uint256 receiveAssetType = platformFacet.getTokenType(
            _depositInfo.receiveAsset
        );
        require(
            sendAssetType != 0 &&
                receiveAssetType != 0 &&
                sendAssetType == _depositInfo.positionType &&
                receiveAssetType == _depositInfo.positionType,
            "VaultPaymaster:asset must be platform allowed"
        );
        require(
            _depositInfo.receiveAsset == weth,
            "VaultPaymaster:receiveAsset must be weth"
        );
        IERC20(_depositInfo.sendAsset).safeTransferFrom(
            msg.sender,
            address(this),
            _depositInfo.amountIn
        );

        IExchangeAdapter.AdapterCalldata
            memory adapterCalldata = IExchangeAdapter(IPlatformAdapter)
                .getAdapterCallData(
                    address(this),
                    _depositInfo.sendAsset,
                    _depositInfo.receiveAsset,
                    _depositInfo.adapterType,
                    _depositInfo.amountIn,
                    _depositInfo.amountLimit,
                    _depositInfo.adapterData
                );
        IERC20(_depositInfo.sendAsset).approve(
            adapterCalldata.spender,
            _depositInfo.approveAmount
        );
        (bool success, bytes memory result) = adapterCalldata.target.call{
            value: adapterCalldata.value
        }(adapterCalldata.data);
        if (!success) {
            assembly {
        ...
        IPaymasterFacet(diamond).setWalletPaymasterBalance(
            _depositInfo.wallet,
            allBalance,
            true
        );
    ...
    function depositEth(address _wallet) external payable nonReentrant {
    ...
    function _validatePaymasterUserOp(
        ...
        return (abi.encode(userOp.sender, userOp.paymasterAndData), 0);
    }
    ...
    function _postOp(
        ...
        require(IPlatformFacet(diamond).getIsVault(sender),"VaultPayMaster:only Vault");
        uint256 quota = paymasterFacet.getQuota(wallet);
        if (quota > 0) {
            address payer = paymasterFacet.getPayer();
            uint256 payerBalance = paymasterFacet.getWalletPaymasterBalance(
                payer
            );
            if (payerBalance >= quota) {
                if (quota >= charge) {
                    quota = charge;
                    charge = 0;
                } else {
                ...
                paymasterFacet.setQuotaWhiteList(0, wallet, quota);
                paymasterFacet.setWalletPaymasterBalance(payer, charge, false);
            }
        }
        if (charge > 0) {
