...
contract IssuanceModule is ModuleBase, IIssuanceModule, Initializable, UUPSUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function setProxyIssueWhiteList(address _vault,address _issuer,bool _status) external onlyVaultOrManager(_vault){
	...
	function getWhiteListAndMode(address _vault,address _issuer) external view returns(IIssuanceFacet.IssueMode,bool){
		IIssuanceFacet issuanceFacet = IIssuanceFacet(diamond);	
	 ...
	 */
	function issue(address _vault, address payable _from, address[] memory _assets,uint256[] memory _amounts) external payable nonReentrant  onlyVaultOrManager(_vault) {
		IPlatformFacet platformFacet=IPlatformFacet(diamond);
		IVaultFacet vaultFacet= IVaultFacet(diamond);
		IIssuanceFacet issuanceFacet = IIssuanceFacet(diamond);	
		require(issuanceFacet.getIssueMode(_vault) != IIssuanceFacet.IssueMode.Proxy,"IssuanceModule:issue mode error"); 
		
		require(!vaultFacet.getVaultLock(_vault),"IssuanceModule:vault is locked");  
		bool isCheck;
        ...
        for (uint256 i; i < _assets.length; i++) {
			...
			if (_assets[i] ==platformFacet.getEth()) {
				...
				if(isCheck){
                  IVault(_from).invokeApprove(_assets[i],_vault, _amounts[i]);
				}		
	...
	function issueAndProxy(address _vault, address[] memory _assets,uint256[] memory _amounts) external payable  nonReentrant {	   
		   IPlatformFacet platformFacet=IPlatformFacet(diamond);
		   IIssuanceFacet issuanceFacet = IIssuanceFacet(diamond);
		   address _from=msg.sender;
		   require(issuanceFacet.getProxyIssueWhiteList(_vault,_from),"IssuanceModule:from error");
		   IIssuanceFacet.IssueMode mode = issuanceFacet.getIssueMode(_vault);
		   if(mode == IIssuanceFacet.IssueMode.Default){
               ...
               require(_from != address(0) && issuanceFacet.getIssuer(_vault) == _from ,"IssuanceModule:issue mode error"); 
		   }  
		   for (uint256 i; i < _assets.length; i++) {
			   validAsset(_vault, _assets[i]);
			   if (_assets[i] ==platformFacet.getEth()) {
	 ...
	 */
	function redeem(address _vault, address payable _to, uint256[] memory _assetsType,address[] memory _assets,uint256[] memory _amounts) external nonReentrant onlyVaultOrManager(_vault) {      
		IIssuanceFacet issuanceFacet = IIssuanceFacet(diamond);
		IIssuanceFacet.IssueMode mode=issuanceFacet.getIssueMode(_vault);
    ...
    /**
	   proxy  redeem  asset
	 */
    function redeemProxy(address _vault, uint256[] memory _assetsType,address[] memory _assets,uint256[] memory _amounts) external nonReentrant{
	    IVaultFacet vaultFacet= IVaultFacet(diamond);
		require(!vaultFacet.getVaultLock(_vault),"IssuanceModule:vault is locked");  
	    IIssuanceFacet issuanceFacet = IIssuanceFacet(diamond);
		IIssuanceFacet.IssueMode mode=issuanceFacet.getIssueMode(_vault);
		address _to=issuanceFacet.getIssuer(_vault);
		require(mode == IIssuanceFacet.IssueMode.Proxy && _to == msg.sender,"IssuanceModule:redeemProxy error");
		executeRedeem(_vault,payable(_to),_assetsType,_assets,_amounts);
        deleteVaultProxyMode(_vault);
	}

   function executeRedeem(address _vault, address payable _to, uint256[] memory _assetsType,address[] memory _assets,uint256[] memory _amounts) internal {
	 IPlatformFacet platformFacet=IPlatformFacet(diamond);
	 for (uint256 i; i < _assets.length; i++) {
	...
	function deleteVaultProxyMode(address _vault) internal {
		 ...
		 uint256 total= IVaultFacet(diamond).getVaultAllPosition(_vault,_positionTypes).length;
		 if(total == 0){
		   IIssuanceFacet issuanceFacet = IIssuanceFacet(diamond);
	       issuanceFacet.setIssuer(_vault, address(0));
		   issuanceFacet.setIssueMode(_vault,IIssuanceFacet.IssueMode.Default);
		 }       
	}
